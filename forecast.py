...
def forecast_demand(hist: pd.DataFrame, periods: int = 6):
    """
    반환: (result_df, method)
      method: "prophet" | "seasonal_short"(데이터 부족) | "seasonal_noprophet"(미설치/오류)
    """
    if len(hist) >= 24:
        try:
            return _prophet(hist, periods), "prophet"
        except Exception:
            return _seasonal_naive(hist, periods), "seasonal_noprophet"
    return _seasonal_naive(hist, periods), "seasonal_short"
...
COMPARE_COLORS = ["#FF4757", "#2E86DE", "#10AC84", "#F39C12", "#8E44AD"]


def plot_compare(series: dict) -> go.Figure:
    """지역 비교: {지역명: DataFrame[ds, visitors]} 를 한 차트에 겹쳐 그림."""
    fig = go.Figure()
    for i, (name, df) in enumerate(series.items()):
        fig.add_trace(go.Scatter(
            x=df["ds"], y=df["visitors"], mode="lines+markers",
            name=name, line=dict(color=COMPARE_COLORS[i % len(COMPARE_COLORS)], width=2.5),
        ))
    fig.update_layout(
