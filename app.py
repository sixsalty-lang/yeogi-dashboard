...
if live:
    region_opts = ds.region_options(nat)
else:
    nat_sample = ds.sample_national()
    region_opts = {r: c for c, r in zip(nat_sample["signguCd"].unique(),
                                        ds.REGIONS)}

naver_live = bool(naver_id and naver_secret)


def region_selector(label_prefix="📍 분석 지역", key="region_sel"):
    """페이지 상단용 지역 선택. 페이지를 옮겨도 선택이 유지된다."""
    names = list(region_opts.keys())
    default = "강원 강릉시" if "강원 강릉시" in names else names[0]
    if key not in st.session_state or st.session_state[key] not in names:
        st.session_state[key] = default
    suffix = f"(전국 {len(names)}곳 · 입력해 검색)" if live else "(샘플 8곳)"
    sel = st.selectbox(f"{label_prefix} {suffix}", names, key=key)
    return sel, region_opts[sel]
...
if page == "📈 수요 예측":
    ...
    with c1:
        region, region_code = region_selector()
    with c2:
        horizon = st.slider("예측 기간(개월)", 3, 12, 6)

    hist = visitors_for(region, region_code, nat)
    ...
    if method == "prophet":
        ...
        st.warning("데이터 기간이 짧아 통계 예측 모델 대신 **계절성 추세 그래프**로 보여드립니다. "
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    if len(picked) < 2:
        ...
        series = {}
        for name in picked:
            series[name] = visitors_for(name, region_opts.get(name), nat)
