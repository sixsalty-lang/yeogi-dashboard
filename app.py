...
# ══════════════════════════════════════════════════════════════════════════
    ...
    if ranking is None or len(ranking) == 0:
        ...
        with cA:
            basis = st.selectbox("정렬 기준", ["전년 대비 성장률", "방문자 수(볼륨)"])
        with cB:
        ...
        if basis == "전년 대비 성장률":
            ...
            st.markdown(f"### 👥 방문자 볼륨 Top {n_show}")
            st.caption("절대 방문자 수가 많은 지역 순. 성장률은 참고용으로 함께 표시됩니다.")
            vol = ranking.sort_values("방문자", ascending=False).head(n_show)
            st.dataframe(vol[show_cols].style.format(fmt),
                         use_container_width=True, hide_index=True)
