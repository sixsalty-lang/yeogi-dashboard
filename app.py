...
# ══════════════════════════════════════════════════════════════════════════
    ...
    # ── 출퇴근·도심 유입 노이즈 제거용 광역 제외 필터 ──
    prov_names = sorted(set(ds.PROVINCE_BY_CODE.values()))
    metro_default = [p for p in ["서울", "경기", "인천"] if p in prov_names]
    excl = st.multiselect(
        "제외할 광역지역", prov_names, default=metro_default,
        help="'외지인' 지표는 출퇴근·쇼핑 유입도 포함합니다. 강남·서초처럼 "
             "통근 유입이 큰 도심이 관광지처럼 잡히는 것을 막으려면 "
             "수도권 등을 제외하세요.",
    )
    if excl:
        _pref = {c for c, n in ds.PROVINCE_BY_CODE.items() if n in excl}
        rank_src = rank_src[~rank_src["signguCd"].str[:2].isin(_pref)]
        st.caption(f"ℹ️ {', '.join(excl)} 제외 중 — 아래 모든 랭킹에 적용됩니다.")

    # ── 집계 기준: 주말(관광 성격) vs 전체 ──
    if "visitors_weekend" in rank_src.columns:
        agg_basis = st.radio(
            "집계 기준", ["주말 방문자 (관광 성격 · 추천)", "전체 방문자"],
            horizontal=True,
            help="주말(토·일)만 집계하면 평일 출퇴근·업무 유입이 걸러져 "
                 "관광 수요에 훨씬 가깝습니다. 지역을 통째로 빼지 않아도 "
                 "가평·양평 같은 수도권 관광지가 살아납니다.",
        )
        if agg_basis.startswith("주말"):
            rank_src = rank_src.copy()
            rank_src["visitors"] = rank_src["visitors_weekend"]
            st.caption("ℹ️ 주말(토·일) 방문자 합계 기준으로 모든 랭킹을 계산합니다.")
    else:
        st.warning("주말 집계를 쓰려면 data_sources.py 최신 버전(v7)으로 교체 후 "
                   "설정에서 캐시를 새로 받아야 합니다. 지금은 전체 방문자 기준입니다.")
    ...
    if hot is not None:
        base_ym, hot_df = hot
        st.markdown(f"### 🚀 다음 달({next_month}월)에 뜰 여행지")
        st.caption(
            f"작년 같은 달({base_ym}) 데이터 기준. '계절지수'는 그 지역 평소 대비 "
            f"{next_month}월 방문 강세(100 초과 = 평소보다 붐빔), "
            "'방문자 볼륨'은 절대 방문자가 많은 순입니다."
        )
        hA, hB = st.columns([2, 3])
        with hA:
            hot_basis = st.selectbox("정렬 기준", ["계절지수(뜨는 강도)", "방문자 볼륨"],
                                     key="hot_basis")
        with hB:
            n_hot = st.slider("표시 개수", 5, 30, 10, key="n_hot")

        vol_col = hot_df.columns[1]  # "작년 N월 방문자"
        if hot_basis.startswith("계절지수"):
            view = hot_df.head(n_hot)
        else:
            view = hot_df.sort_values(vol_col, ascending=False).head(n_hot)
        fmt = {vol_col: "{:,.0f}", "계절지수": "{:.0f}"}
    ...
    if ranking is None or len(ranking) == 0:
        ...
        n_show = st.slider("표시 개수", 5, 20, 10, key="n_rank")
        st.markdown(f"**기준: {base_month} vs 전년 동월** · 총 {len(ranking)}개 지역")

        show_cols = ["지역", "방문자", "전년동월", "성장률"]
        fmt = {"방문자": "{:,.0f}", "전년동월": "{:,.0f}", "성장률": "{:+.1f}%"}
        up, down = st.columns(2)
        with up:
            st.markdown(f"### 🔥 성장 Top {n_show}")
            st.dataframe(ranking.head(n_show)[show_cols].style.format(fmt),
                         use_container_width=True, hide_index=True)
        with down:
            st.markdown(f"### 🧊 감소 Top {n_show}")
            st.dataframe(ranking.tail(n_show)[show_cols].iloc[::-1].style.format(fmt),
                         use_container_width=True, hide_index=True)
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    st.caption("네이버 검색으로 **최근 3개월 이내** 여행 예능 관련 콘텐츠만 수집합니다.")

    kw = st.text_input("검색 키워드", value="예능 국내여행 여행지")
    cards = cached_spots(kw, naver_id or None, naver_secret or None)

    if not cards:
        st.info("최근 3개월 이내 결과가 없습니다. 키워드를 바꿔보세요.")
    else:
        cols = st.columns(2)
        for i, c in enumerate(cards):
            with cols[i % 2]:
                date_html = (f"<span class='src' style='float:right'>{c['date']}</span>"
                             if c.get("date") else "")
                st.markdown(
                    f"""
                    <div class='metric-card' style='margin-bottom:14px'>
                        <div class='tag'>{c['program']}</div>{date_html}
    ...
    st.caption("출처: 네이버 검색 API(최신순, 90일 이내). 자동 수집 결과로, 실제 방영 정보와 다를 수 있습니다.")
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("### 📡 데이터 상태")
    ds_ver = getattr(ds, "VERSION", "v5 이하(구버전!)")
    st.markdown(
        f"- 코드 버전: app **v7** / data_sources **{ds_ver}** "
        f"{'✅' if ds_ver == 'v7' else '⚠️ 파일 교체가 누락됐습니다'}\n"
