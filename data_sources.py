...
def _fetch_naver_blog(keyword: str, cid: str, secret: str):
    """최신순으로 넉넉히 받아 최근 90일 이내 글만 남긴다 (최대 20건)."""
    ...
    for it in r.json().get("items", []):
        postdate = str(it.get("postdate", ""))
        if postdate and postdate < cutoff:
            continue  # 90일 이전 글 제외
        ...
        out.append({
            ...
            "desc": desc,
            "link": link,
            "date": f"{postdate[:4]}.{postdate[4:6]}.{postdate[6:]}" if len(postdate) == 8 else "",
        })
        if len(out) >= 20:
            break
    return out
