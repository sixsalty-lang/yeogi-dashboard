...
def fetch_national_monthly(key: str, months: int = 36) -> Optional[pd.DataFrame]:
    ...
    acc: dict[tuple, float] = {}    # (ym, cd, name) -> 전체 합계
    acc_w: dict[tuple, float] = {}  # (ym, cd, name) -> 주말(토·일) 합계
    days_seen: dict[str, set] = {}  # ym -> 데이터가 존재하는 날짜 집합 (완성도 판별용)
    _wk_memo: dict[str, bool] = {}  # ymd -> 주말 여부 (반복 계산 절약)
    ...
    while cur <= end:
        ...
        while True:
            ...
            for it in rows:
                ...
                v = float(it.get("touNum", 0))
                acc[(ym, cd, name)] = acc.get((ym, cd, name), 0.0) + v
                # 주말(토·일)만 별도 누적 -> 출퇴근 유입이 걸러진 관광 성격 지표
                if ymd not in _wk_memo:
                    try:
                        _wk_memo[ymd] = dt.datetime.strptime(ymd, "%Y%m%d").weekday() >= 5
                    except ValueError:
                        _wk_memo[ymd] = False
                if _wk_memo[ymd]:
                    acc_w[(ym, cd, name)] = acc_w.get((ym, cd, name), 0.0) + v
    ...
    return pd.DataFrame({
        ...
        "visitors_weekend": [acc_w.get(k, 0.0) for k in keys],
    })
...
def sample_national() -> pd.DataFrame:
    ...
    for i, r in enumerate(REGIONS):
        ...
        df["visitors_weekend"] = (df["visitors"] * 0.35).round().astype(int)
        frames.append(df)
    return pd.concat(frames, ignore_index=True)[
        ["ds", "signguCd", "signguNm", "visitors", "visitors_weekend"]]
